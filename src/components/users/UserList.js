            ...
            {users.map(user => (
                <div key={user?.key}>
                    <a className="flex justify-between items-center px-3 py-2" href="/#">
                        <span className="text-sm text-gray-700 font-medium dark:text-gray-400">{user?.value.name}</span>
                        <span ref={activeRef} className={`text-sm font-semibold ${user?.value.isActive ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-500'}`}>
